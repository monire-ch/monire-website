    ...
    <section id="pricing" className="dark-teal-surface py-20 md:py-28 px-6 relative">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <ScrollReveal className="flex justify-center mb-10">
          <div className="inline-flex rounded-full border border-off-white/20 p-1.5 gap-1 w-full max-w-md md:w-auto">
            {tabKeys.map((key) => (
              <button
                key={key}
                onClick={() => setActiveTab(key)}
                className={`flex-1 md:flex-none px-6 md:px-8 py-3 rounded-full text-xs md:text-sm font-body transition-all duration-200 ${
                  activeTab === key
