...
import type { AiCaseStudyConfig } from '@/config/caseStudies';

type AiCaseStudyTemplateProps = {
  project: AiCaseStudyConfig;
};

const AiCaseStudyTemplate = ({ project }: AiCaseStudyTemplateProps) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 mb-16">
            <ScrollReveal>
              <div className="rounded-xl overflow-hidden bg-card border border-border">
                <img src={project.image} alt={project.imageAlt} className="w-full h-auto" />
              </div>
            </ScrollReveal>
            ...
            <div className="space-y-8">
              {project.sections.map((section) => (
                <ScrollReveal key={section.title}>
                  ...
                  <p className="text-sm font-body text-foreground/80 mb-2">{section.intro}</p>
...
export default AiCaseStudyTemplate;
